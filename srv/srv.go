...
func (s *Server) pullDataFromSensor() {
	...
	defer func() {
		if err != nil {
			...
			// Send ntfy notification for sensor error
			if s.notifier != nil {
				if notifyErr := s.notifier.SendError(err.Error()); notifyErr != nil {
					log.Erro.Printf("failed to send ntfy notification: %s", notifyErr.Error())
				}
			}
		}
	}()
